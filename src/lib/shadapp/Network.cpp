...
namespace shadapp {
    ...
    void Network::initQtSignals(shadapp::fs::Device *device) {
        ...
        connect(device->getSocket(), SIGNAL(readyRead()),
                device, SLOT(slotReceive()));
        connect(device, SIGNAL(signalReceive(shadapp::fs::Device*)),
                this, SLOT(slotReceive(shadapp::fs::Device*)));
    ...
    void Network::slotSocketConnected(shadapp::fs::Device *connectedDevice) {
        ...
        shadapp::protocol::ClusterConfigMessage message(
                *lp->getConfig()->getVersion(),
                ...
                messageFolders,
                options);
        std::cout << "CCM envoye a " << connectedDevice->getId() << "sizeOf : " << sizeof (message) << std::endl;
        send(connectedDevice->getSocket(), message);
    }
    ...
    void Network::slotReceiveCCMfromNewPeer() {
        ...
        //this->clusterConfigUpdate(ccm.getFolders());
    ...
    void Network::clusterConfigUpdate(std::vector<shadapp::fs::Folder> msgFolders) {
        //        for (auto folder_temp : msgFolders) {
        //            bool newFolder = true;
        //            for (auto localFolder_temp : config->getFolders()) {
        //                if (folder_temp.getId() == localFolder_temp.getId()) {
        //                    newFolder = false;
        //                    bool newDevice = true;
        //                    for (auto device_temp : folder_temp.getDevices()) {
        //                        for (auto &localDevice_temp : localFolder_temp.getDevices()) {
        //                            if (device_temp->getId() == localDevice_temp->getId()) {
        //                                newDevice = false;
        //                            }
        //                        }
        //                        if (newDevice) {
        //                            localFolder_temp.addDevice(device_temp);
        //                        }
        //                    }
        //                }
        //            }
        //            if (newFolder) {
        //                config->addFolder(folder_temp);
        //            }
        //        }
